...
@section('content')
    ...
    <div class="py-5 px-5">
        <div class="text-2xl font-bold ">Condition of DCP Items</div>
        <div class="text-lg font-normal text-gray-500 mb-4">Here are the List of Items</div>
        <div class="py-5">
            <span class="text-sm text-gray-600">You Can Filter By Condition</span>
            <form id="conditionForm" action="{{ route('schools.item.condition.combo') }}" method="POST">
                @csrf
                @method('POST')
                <select
                    class="     py-1 px-2 border border-gray-300 bg-white rounded-sm shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
