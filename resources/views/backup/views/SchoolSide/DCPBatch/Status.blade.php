...
@section('content')
    <div class="mx-5 mt-5 border-1">
        ...
        <div class="overflow-x-auto border-t border-gray-800 h-96 mt-4 rounded-sm md:border-b shadow-md md:shadow-none">
            <table class="w-full bg-white border-collapse    ">
                <thead class="bg-gray-200 border border-gray-800 sticky top-0 z-50">
                    <tr>
                        <td
                            class="px-4 py-2  border-b border-gray-800 font-semibold whitespace-nowrap tracking-wider text-gray-800">
                            Item Code</td>
                             <td
                            class="px-4 py-2  border-b border-gray-800 font-semibold whitespace-nowrap tracking-wider text-gray-800">
                            Item </td>
                        ...
                        <td
                            class="px-4 py-2  border-b border-gray-800  font-semibold  tracking-wider whitespace-nowrap text-gray-800">
                            Condition upon Delivery</td>
                        <td class="px-4 py-2  border-b border-gray-800   font-semibold  tracking-wider text-gray-800">Brand
                        </td>
                        <td class="px-4 py-2   border-b border-gray-800  font-semibold  tracking-wider text-gray-800">Serial
                        </td>
                        <td class="px-4 py-2   border-b border-gray-800  font-semibold  tracking-wider text-gray-800">
                            Functional
                        </td>
                ...
                <tbody>
                    @foreach ($items as $index => $item)
                        <tr>
                            ...
                            <td class="px-4 py-3 border border-gray-300"> @if ($item->item_status == 1) Functional @else Not Functional @endif </td>
                            ...
                            <td class="px-4 py-3 whitespace-nowrap border text-center  ">
                                ...
                                <a href="{{ route('school.dcp_item_warranty', $item->pk_dcp_batch_items_id) }}"
                                    class="bg-green-200 text-gray-800 rounded    hover:bg-green-600 hover:text-white px-2 border border-gray-800   py-1">
                                    Show Warranty
                                </a>
                            </td>
                        </tr>
        ...
        <div id="user-recipient-modal-overlay"
            ...
            <div id="user-recipient-modal"
                class="bg-white rounded-lg shadow-xl   px-5 py-2 w-full mx-5 max-w-2xl relative">
                ...
                <form id="user-recipient-form" method="POST" action="{{ route('school.assignment.items') }}">
                    ...
                    <div class="mt-4 flex justify-end gap-2  ">
                        <button type="submit"
                            class="bg-blue-600 w-40 text-white px-4 py-1 rounded hover:bg-blue-700">Save
                            Information</button>
                        <button type="button" onclick="closeUserRecipientModal()"
    ...
    <script>
        function openUserRecipientModal(item) {
            ...
            if (item.isAssigned) {
                ...
                document.getElementById('modal_pk_dcp_batch_items_id').value = '';
                document.getElementById('modal_assigned_user_type_id').value = '';
                document.getElementById('modal_assigned_user_name').value = '';
                document.getElementById('modal_assigned_user_location_id').value = '';
            }
